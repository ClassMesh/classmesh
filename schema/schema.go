// Package schema implements a classmesh.Stage that validates a structured record's
...
func missingReason(detail string) classmesh.Reason {
	return classmesh.Reason{Code: "missing", Detail: detail}
}
func typeReason(detail string) classmesh.Reason {
	return classmesh.Reason{Code: "type", Detail: detail}
}
...
// Classify implements classmesh.Stage: a record violating the schema is classified
...
func (s *Stage) Classify(ctx context.Context, r classmesh.Record) (classmesh.Classification, error) {
	...
	return classmesh.Classification{Category: s.category, Confidence: 1, Stage: Name, Reasons: out}, nil
}

// classifyTrie walks the trie once, recording violations by declaration index,
// then restores declaration order.
func (s *Stage) classifyTrie(fields map[string]any) []classmesh.Reason {
	var acc reasons
	...
	acc.sort()
	out := make([]classmesh.Reason, len(acc.pending))
...
func appendReason(out []classmesh.Reason, r classmesh.Reason) []classmesh.Reason {
	if out == nil {
		out = make([]classmesh.Reason, 0, firstViolationCap)
	}
	return append(out, r)
}
...
func (a *reasons) add(index int, r classmesh.Reason) {
	if a.pending == nil {

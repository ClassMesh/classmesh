...
func TestValidRecordEscalates(t *testing.T) {
	...
	if _, err := s.Classify(context.Background(), event(map[string]any{"level": "error", "status": float64(500)})); !errors.Is(err, domain.ErrUnclassified) {
		t.Fatalf("valid record error = %v, want ErrUnclassified", err)
	}
	if _, err := s.Classify(context.Background(), event(map[string]any{"level": "info"})); !errors.Is(err, domain.ErrUnclassified) {
...
func TestNumberAcceptsJSONNumber(t *testing.T) {
	s := mustNew(t, Field{Path: "status", Type: Number})
	if _, err := s.Classify(context.Background(), event(map[string]any{"status": json.Number("500")})); !errors.Is(err, domain.ErrUnclassified) {
...
func TestTrieValidRecordEscalates(t *testing.T) {
	...
	if _, err := s.Classify(context.Background(), event(fields)); !errors.Is(err, domain.ErrUnclassified) {
		t.Fatalf("valid record error = %v, want ErrUnclassified", err)
	}
}
...
func classifyNaive(s *Stage, fieldsMap map[string]any) ([]domain.Reason, error) {
	...
	if out == nil {
		return nil, domain.ErrUnclassified
	}
	return out, nil
}
